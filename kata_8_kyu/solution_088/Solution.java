package com.codewars.kata_8_kyu.solution_088;
...
/*
Gravity Flip
f you've completed this kata already and want a bigger challenge, here's the 3D version
...
public class Solution {
    public static int[] flip(char dir, int[] arr) {
        if (dir != 'R' && dir != 'L') return new int[0];
        Arrays.sort(arr);
        int[] flipArray = new int [arr.length];
        if (dir == 'L') {
            int flipCount = arr.length - 1;
            for (int i = 0; i < arr.length; i++) {
                flipArray[i] = arr[flipCount];
                flipCount--;
            }
            return flipArray;
        }
